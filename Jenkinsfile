        ...
        stage('Building code') {
            steps {
                script {
                    ...
                    jiraTransitionIssue idOrKey: 'CT-13', input: transitionInput
            ...
            post {
                success {
                    script{
                        withEnv(['JIRA_SITE=jira']) {
                            def comment = [ body: 'Hello-world app build succeeded' ]
                            jiraAddComment idOrKey: 'CT-13', input: comment
                        }
                    }
                }
                failure {
                    ...
                    script {
                        withEnv(['JIRA_SITE=jira']) {
                            def failureIssue = [fields: [ project: [id: '10000'],
                                                        summary: 'Failed build',
                                                        description: 'Check the causes of failure to Build URL for details',
                                                        issuetype: [id: '10005']]]
                            response = jiraNewIssue issue: failureIssue
